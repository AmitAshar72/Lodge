...
bool SoundClass::Initialize(HWND hwnd)
{
	...
	result = LoadWaveFile("../Engine/data/ambient.wav", &m_secondaryBuffer, &m_secondary3DBuffer);
	if (!result)
	{
		return false;
	}
	// Play the wave file now that it has been loaded.
	result = PlayWaveFile(&m_secondaryBuffer, &m_secondary3DBuffer, 0.0f, 4.85f, 15.0f);
	if (!result)
	{
		return false;
	}
	...
	// Load a wave audio file onto a secondary buffer.
	result = LoadWaveFile("../Engine/data/music.wav", &m_secondaryBuffer1, &m_secondary3DBuffer1);
	if (!result)
	{
		return false;
	}
	...
	// Play the wave file now that it has been loaded.
	result = PlayWaveFile(&m_secondaryBuffer1, &m_secondary3DBuffer1, -10.0f, 3.66f, 2.0f);
	if (!result)
	{
		return false;
	}
...
void SoundClass::Shutdown()
{
	...
	ShutdownWaveFile(&m_secondaryBuffer, &m_secondary3DBuffer);
	//Shutdown2DWaveFile(&m_secondaryBuffer);
...
void SoundClass::setListenerPosition(float x, float y, float z)
{
	...
	lisY = y;
	lisZ = z;
	m_listener->SetPosition(lisX, lisY, lisZ, DS3D_IMMEDIATE);
}
...
bool SoundClass::PlayWaveFile(IDirectSoundBuffer8** secondaryBuffer, IDirectSound3DBuffer8** secondary3DBuffer, float x, float y, float z)
{
	HRESULT result;
